...
According to the plot, the majority of laptops in our dataset are priced between \$250 and $4300. The higher-priced laptops are probably larger and heavier and have far more sophisticated features.
...
The following features have a significantly impact the price:

```Processor Brand``` : AMD, Intel

```Processor Name``` : i3, i5, i7, GEForce GTX
...
```Touchscreen``` : Having a touchscreen are significant to the price.
...
```
We split the data into  80% training and 20% testing as we felt that would be the best way to approach training and testing our models.
...
In this step we are adding the recipe to the lasso model. We are also making the workflow and grid for the `tune_grid`. We use the folds we did earlier for the `tune_grid` also. The plots seem to increase/decrease quickly compared to the Ridge Model.
...
In this step we are adding the recipe to the Tree model. We are also making the workflow and grid for the `tune_grid`. We use the folds we did earlier for the `tune_grid` also. The `cost_complexity` parameter seems to have more similarities to the lasso plots. The difference is the plots are not smooth, and seem to have sudden changes of slope.
...
```RMSE``` : mean = 269 & standard error = 9.37
```RSQ``` : mean = 0.777 & standard error = 0.0149
...
```RMSE``` : mean = 271 & standard error = 9.34
```RSQ``` : mean = 0.774 & standard error = 0.0143
...
```RMSE``` : mean = 288 & standard error = 9.84
```RSQ``` : mean = 0.750 & standard error = 0.0150
...
```RMSE``` : mean = 332 & standard error = 11.5
```RSQ``` : mean = 0.701 & standard error = 0.0151

Looking at all the model's rmse and rsq, Ridge would be the best model to test on the Testing Set. Boost has the lowest RMSE and highest RSQ.
...
Before analyzing the results of the rmse and rsq from the training set, we thought the tree model would do best because most of our data was categorical. We believe the reason why the tree model did poorly was because we think the tree model is more successful when the variable we are predicting is categorical also. Lasso and Ridge came out better than tree, because Lasso and Ridge are more flexible than the other two models we fitted. Boost came out on top because it reduced the bias the best without significantly increasing the variance.

This research shows Boost would be the best model. This model is the best at predicting the price of a laptop, based on the features the laptop has. Predicting with this model, will give the customer a price with around 77% accuracy. The goal of our project was achieved because we got a model with an accuracy over 70%.

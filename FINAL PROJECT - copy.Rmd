...
laptop <- subset(laptop, latest_price != "Missing") # 354 observations now
```
...
dim(ec_train) #282 obs. 18 columns
ec_test <- testing(ec_split)
dim(ec_test) #72 obs. 18 columns
```

### Making the recipe and folds

```{r}
ec_recipe <- recipe(latest_price ~ brand + model + processor_brand + processor_name + processor_gnrtn + ram_gb + ram_type + ssd + hdd + os + os_bit + graphic_card_gb + weight + display_size + warranty + touchscreen + msoffice, data = ec_train) %>%
